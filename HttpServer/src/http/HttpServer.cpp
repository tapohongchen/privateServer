                           ...
                           muduo::Timestamp receiveTime)
{
    try
    {
        ...
        //conn->getMutableContext()用来获取前面绑定的httpcontext（conn->setContext(HttpContext());）
...
void HttpServer::handleRequest(const HttpRequest &req, HttpResponse *resp)
{
    try
    {
        ...
        // 通过里面的throw response;把响应报文抛出，在catch中捕获并处理
        middlewareChain_.processBefore(mutableReq);

        // 路由处理，当找到对应函数就调用并返回true
